...
def convert_forward_solution(fwd, surf_ori=False, force_fixed=False,
    ...
    if is_fixed_orient(fwd, orig=True) or (force_fixed and not use_ave_nn):
        ...
        source_nn_cortex = np.concatenate(
                [s['nn'][s['vertno'], :] for s in fwd['src'][:2]], axis=0)
        ...
        fwd['source_nn'] = np.concatenate(
                (source_nn_cortex, source_nn_aseg), axis=0)
        ...
        if n_col != fwd['source_nn'].shape[0]:
            raise ValueError(
                    'mismatch btw ncol and fwd[''source_nn''].shape[0]')
